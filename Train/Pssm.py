    ...
    for size in winsizelist:
        ...
        for train,test in skf.split(data,label):
            ...
            model.fit(x_train,y_train,batch_size=100,epochs=100,
                      verbose=2)
            model.save('Result/Train/DifferentRadius/pssm/pssm'+str(size)+'_'+ str(modelIdx)+'.h5')
...
def TrainDNN():
    ...
    for train, test in skf.split(data, label):
        ...
        y_train = label[train];
        y_test = label[test]
        model = CNNModel.DNN(shape=(WINSIZE * 2 + 1, 20))
        modelIdx += 1
        model.fit(x_train, y_train, batch_size=100, epochs=100,
                  verbose=2)
        model.save('Result/Train/CompaDNN/pssm/pssm' + str(WINSIZE) + '_' + str(modelIdx) + '.h5')
        y_pre = model.predict(x_test)
        p_classes = [int(prob > 0.5) for prob in y_pre.flatten()]
